...
def main() -> None:
    ...
    model_dict = {
        "cpc": contrastlib.ContrastivePredictiveModel,
    }
    model = model_dict[args_model](**config[f"{args_model}_params"])

    train_data_pos = contrastlib.SequentialMNIST(
        ordered=True, root=data_dir, train=True, download=True, **config["dataset_params"]
    )
    train_data_neg = contrastlib.SequentialMNIST(
        ordered=False, root=data_dir, train=True, download=True, **config["dataset_params"]
    )
    test_data_pos = contrastlib.SequentialMNIST(
        ordered=True, root=data_dir, train=False, download=True, **config["dataset_params"]
    )
    test_data_neg = contrastlib.SequentialMNIST(
        ordered=False, root=data_dir, train=False, download=True, **config["dataset_params"]
    )

    trainer = contrastlib.Trainer(**params)
...
def init_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default="cpc", help="Model name.")
